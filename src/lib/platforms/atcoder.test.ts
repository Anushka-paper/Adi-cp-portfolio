...
describe("atcoderAdapter", () => {
  ...
  it("normalizes an AtCoder profile", async () => {
    ...
    expect(profile.ratingHistory).toHaveLength(2); // unrated entry excluded
    expect(profile.currentRating).toBe(1250);
    expect(profile.maxRating).toBe(1250);
    expect(profile.rank).toBe("cyan"); // 1250 falls in the cyan tier
