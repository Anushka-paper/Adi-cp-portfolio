...
- [x] OG image generation — `src/app/opengraph-image.tsx` via `next/og`'s `ImageResponse` (`runtime = "nodejs"`, since the `postgres` driver needs real Node sockets, not Edge-compatible), showing avatar/name/role/current ratings per synced platform/rank badge in the same accent color as the profile card. `metadataBase` added to the root layout (`VERCEL_PROJECT_PRODUCTION_URL`, falls back to localhost) so the image resolves to a real absolute URL in share previews instead of defaulting to localhost. Same staleness consideration as the home page — also statically prerendered, so `runSync()` now revalidates `/opengraph-image` alongside `/`. Verified by fetching the generated PNG directly against a real production build.
